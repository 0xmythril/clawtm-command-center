...
import { Play, Pause, RefreshCw, Clock, Loader2, Info, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
function formatSchedule(job: CronJob): { short: string; human: string } {
  ...
  if (schedule.kind === "cron" && schedule.expr) {
    return {
      short: schedule.expr,
      human: humanizeCron(schedule.expr),
    };
  }
...
function getPayloadDescription(job: CronJob): string {
  ...
  if (payload.kind === "systemEvent") {
    return payload.text || payload.message || "System event";
  }
  if (payload.kind === "agentTurn") {
    return payload.message || payload.text || "Agent turn";
  }
  return payload.kind;
}
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400">
            {cronStatus?.enabled ? "Scheduler active" : "Scheduler paused"} · {enabledCount}/{cronJobs.length} enabled
          </p>
        </div>
        <button
          onClick={refresh}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="group relative">
                  ...
                  <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-zinc-800 rounded-lg text-xs text-zinc-300 shadow-lg z-10">
                    When the scheduler will next wake up to run any enabled job. This is the soonest scheduled job.
                  </div>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div className="flex gap-4 mt-2 text-xs text-zinc-500">
                      ...
                      {job.state?.lastDurationMs && (
                        <span>Took: <span className="text-zinc-400">{Math.round(job.state.lastDurationMs / 1000)}s</span></span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-zinc-800 space-y-3">
                    ...
                    <div className="flex gap-4 text-xs">
                      ...
                      <div>
                        ...
                        <span className="text-zinc-400">{job.wakeMode}</span>
                      </div>
                    </div>
                    {job.schedule.tz && (
                      <div className="text-xs">
                        <span className="text-zinc-500">Timezone: </span>
                        <span className="text-zinc-400">{job.schedule.tz}</span>
                      </div>
                    )}
                  </div>
                )}
